...
export default function App() {
  ...
  useEffect(() => {
    // Load saved model preference, then detect what's available
    const saved = localStorage.getItem('fags_agent_llm');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (parsed.provider && parsed.model) setConfig(parsed);
      } catch {}
    }
    detectProvider().then((r) => {
      setDetect(r);
      // Only override with detected if user hasn't saved a preference
      if (!saved) setConfig(r.config);
    });
  }, []);
    ...
    <div className="min-h-dvh bg-neutral-950 text-neutral-100 flex flex-col">
      <header className="flex items-center gap-3 px-4 py-3 border-b border-neutral-800 shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <a href="https://freeagentstore.online/console/#keys" className="text-xs text-neutral-600 hover:text-neutral-400">Keys</a>
        </div>
      </header>
      ...
      {/* Model selector — always visible when not running */}
      {!isRunning && !isPaused && (
        <div className="border-b border-neutral-800 bg-neutral-900/50 px-4 py-2">
          <div className="flex gap-3 items-center flex-wrap max-w-4xl mx-auto">
            <span className="text-xs text-neutral-500 font-semibold">Model:</span>
            <select value={config.provider} onChange={(e) => {
              const p = PROVIDERS.find((x) => x.id === e.target.value);
              const next = { provider: e.target.value, model: p?.models[0] ?? '' };
              setConfig(next);
              localStorage.setItem('fags_agent_llm', JSON.stringify(next));
            }} className="px-2 py-1 rounded-lg bg-neutral-950 border border-neutral-700 text-xs">
            ...
            {detect && (
              <span className="text-[10px] text-neutral-600 ml-auto">
                {detect.hasSession ? 'Signed in' : 'Not signed in'}{detect.hasOllama ? ' | Ollama' : ''}
              </span>
            )}
          </div>
        </div>
