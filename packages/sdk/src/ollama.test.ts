...
describe('OllamaClient', () => {
  ...
  it('detect() returns available:true when Ollama responds', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ...
      json: () => Promise.resolve({
        models: [
          { name: 'llama3.2', size: 2_000_000_000, details: { parameter_size: '3B', quantization_level: 'Q4_0' } },
        ],
      }),
    }));
  ...
  it('generate() throws on API error', async () => {
    ...
    await expect(client.generate('llama3.2', 'hello')).rejects.toThrow('Ollama generate failed: 500');
  });
